...
public class PIDController {
    ...
    private int msdelay;
    private boolean loggerEnbale = false;
    ...
    public int doPID(int processVariable){
        ...
        if (this.loggerEnbale&&cycleCount>2) {
            Logger.info("PID", "SP="+setpoint);
            Logger.info("PID", "MV="+outputMV);
            Logger.info("PID", "PV="+processVariable);
            Logger.info("PID", "Integral="+integral);
            Logger.info("PID", "Kp*error="+Kp*error);
            Logger.info("PID", "Kd*derivative="+Kd*derivative);
            Logger.info("PID", "error="+error);
            Logger.info("PID", "dt="+dt);
        }
    ...
    /**
     * Enable logger before <code>doPID()</code> returns.
     */
    public void enableDataLogger(){
        this.loggerEnbale=true;
    }

    /**
     * Disable logger before <code>doPID()</code> returns.
     */
    public void diasbleDataLogger(){
    	this.loggerEnbale=false;
    }
